...
public class Road2Script : MonoBehaviour
{
    ...
    {
        spawnTrain(ObjectPool.Instance.poolTrain[2].GetPooledObject(Resources.Load<GameObject>("Train/Train3")),new Vector3(-3,0,transform.position.z-65));
        spawnTrain(ObjectPool.Instance.poolTrain[0].GetPooledObject(Resources.Load<GameObject>("Train/Train1")),new Vector3(0,0,transform.position.z-40));
        spawnTrain(ObjectPool.Instance.poolTrain[0].GetPooledObject(Resources.Load<GameObject>("Train/Train1")),new Vector3(-3,0,transform.position.z-20));
        
    }
    public void spawnCoininRoad()
    {
        transformCoin = new Vector3(3, 1, transform.position.z-22);
        for (int i = 0; i < 7; i++)
        {
        ...
        transformCoin = new Vector3(-3, 4, transform.position.z-22);
        for (int i = 0; i < 5; i++)
        {
            spawnCoin(transformCoin);
            transformCoin += new Vector3(0, 0, pConz);
    ...
    {
        SpawnBlocker(ObjectPool.Instance.PoolBlocker[0].GetPooledObject(Resources.Load<GameObject>("Blocker/Block" + 1)), new Vector3(3, 0.8f, transform.position.z - 30));
    }
